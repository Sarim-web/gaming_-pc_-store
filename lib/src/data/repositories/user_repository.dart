...
class UserRepository {
  ...
  User _parseUserResponse(http.Response res) {
    if (res.statusCode != 200) {
      final String body = res.body.trim();
      try {
        final dynamic decoded = jsonDecode(body);
        throw Exception(decoded['msg'] ?? 'Server returned ${res.statusCode}');
      } catch (_) {
    ...
    try {
      ...
      return User.fromJson(jsonEncode(decoded));
    } catch (e) {
      throw Exception('Invalid user response: ${e.toString()}');
    }
  }

  Future<User> getUserDataInitial(String token) async {
    try {
      final http.Response res = await userApi.getUserDataInitial(token);
      return _parseUserResponse(res);
    } catch (e) {
  ...
  Future<User> getUserData() async {
    try {
      final http.Response res = await userApi.getUserData();
      return _parseUserResponse(res);
    } catch (e) {
      throw Exception(e.toString());
    }
